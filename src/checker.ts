 ...
 */
function matchesForbiddenPath(filePath: string, forbiddenPaths: string[]): boolean {
 ...
 * 禁止エリアに変更があるファイルを抽出
 */
function findForbiddenFiles(changedFiles: string[], forbiddenPaths: string[]): string[] {
	return changedFiles.filter((file) => matchesForbiddenPath(file, forbiddenPaths));
}
