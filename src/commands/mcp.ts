		...
		async ({ force, dryRun, args }) => {
			try {
				...
				if (!checkResult.allowed) {
					...
					if (config.onForbidden === "prompt" && details.hasForbiddenChanges) {
						message +=
				...
				if (result.success) {
					return {
						content: [
							{
								type: "text" as const,
								text: result.output ? `Push successful\n${result.output}` : "Push successful",
							},
						],
					};
				}
...
export function createMcpCommand(): Command {
	const cmd = new Command("mcp")
		...
		.action(async (opts: { http?: boolean; port?: string }) => {
			if (opts.http) {
				const port = opts.port ? Number(opts.port) : Number(process.env.PORT) || 3000;
