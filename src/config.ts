	...
	try {
		...
		if (errors.length > 0) {
			const errorMessages = errors
				...
				.join(", ");
			throw new ConfigError(`Failed to parse config file: ${errorMessages}`, filePath);
		}
 ...
 */
export function initConfig(configPath?: string, force = false): { created: boolean; path: string } {
	const filePath = configPath ?? getConfigPath();
